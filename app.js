...
if (formDom) { // Only run this if we are on domestic.html
...
if (intForm) { // Only run this if we are on international.html
    ...
    intForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if(result.result === 'success') {
                ...
                intForm.reset();
                boxContainer.innerHTML = ''; boxCount = 0; addBox(); window.calculateTotals();
            } else alert('Error: ' + result.error);
        } catch (error) {
            alert('Transmission failed.');
        } finally {
...
if (tableBody) { // Only run this if we are on processing.html
    ...
    window.fetchDomesticBookings = async function() {
        ...
        try {
            ...
            if (result.result === 'success') {
                ...
                tableBody.innerHTML = ''; 
                if (data.length <= 1) {
                    tableBody.innerHTML = '<tr><td colspan="6" class="p-8 text-center text-slate-500">No bookings found.</td></tr>';
                    return;
                }
                for (let i = data.length - 1; i > 0; i--) {
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="p-4 border-b border-slate-800"><span class="px-2 py-1 rounded text-xs font-semibold ${statusColor}">${row[9]}</span></td>
                    `;
                    tableBody.appendChild(tr);
                }
            } else tableBody.innerHTML = `<tr><td colspan="6" class="p-8 text-center text-red-400">Error: ${result.message}</td></tr>`;
        } catch (error) {
            tableBody.innerHTML = `<tr><td colspan="6" class="p-8 text-center text-red-400">Failed to fetch data.</td></tr>`;
        }
    }
    
    refreshBtn.addEventListener('click', window.fetchDomesticBookings);
    window.fetchDomesticBookings(); // Auto-fetch on load
}